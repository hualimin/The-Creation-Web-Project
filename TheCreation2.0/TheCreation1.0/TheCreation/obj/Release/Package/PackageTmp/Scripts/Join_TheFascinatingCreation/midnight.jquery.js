			...
			var setupHeaders = function() {
				...
				if ($customHeaders.length) {
					if (!$customHeaders.filter('.' + settings['defaultClass']).length) {
						...
						$customHeaders.filter('.' + settings['headerClass'] + ':first').clone(true, true).attr('class', settings[
							'headerClass'] + ' ' + settings['defaultClass']);
					}
				} else {
					// If there are no custom headers, just wrap the content and make that the default header
					$originalHeader.wrapInner('<div class="' + settings['headerClass'] + ' ' + settings['defaultClass'] +
						'"></div>');
				}
				...
				for (headerClass in headers) {
					if (typeof headers[headerClass].element === 'undefined') {
						...
						if ($existingHeader.length) {
							...
							headers[headerClass].element = $defaultHeader.clone(true, true).removeClass(settings['defaultClass']).addClass(
								headerClass).appendTo($originalHeader);
						}
